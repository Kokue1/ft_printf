...
/*   Updated: 2021/02/22 19:08:48 by flemos-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/printf.h"

static char	*ft_write_w_m_x(t_format *struct_, char *trade)
{
	int		i;
	int		j;
	char	*temp;
...
static char	*ft_write_w_x(t_format *struct_, char *trade)
{
	int		i;
	int		j;
	char	*temp;
...
static char	*ft_write_zero_x(t_format *struct_, char *trade)
{
	char	*temp;
	...
	{
		temp = ft_wsp_x(struct_, trade, temp);
		return (temp);
	}
	temp = ft_z_cut_func_x(struct_, trade, temp);
	return (temp);
}

static char	*ft_verify_p_x(t_format *struct_, char *trade)
{
	char	*temp;
	int		len;
	...
	if (struct_->precision < 0 || struct_->precision < (int)ft_strlen(trade))
		return (trade);
	ft_verify_p_xx(struct_, trade);
	len = (int)ft_strlen(trade) + struct_->precision + 1;
	temp = malloc(sizeof(char) * (len));
	if (temp == NULL)
		return (NULL);
	temp = ft_write_p_x(struct_, trade, temp);
	return (temp);
}
...
void	ft_write_hexa(t_format *struct_, char *trade)
{
	...
	if (struct_->precision == 0)
		trade = ft_pre_z_x(trade);
	else
		trade = ft_verify_p_x(struct_, trade);
	if (struct_->flag == '0')
		trade = ft_write_zero_x(struct_, trade);
	if (struct_->flag == '-')
		trade = ft_write_w_m_x(struct_, trade);
	else
		trade = ft_write_w_x(struct_, trade);
